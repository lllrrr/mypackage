...
function check_update()
		...
		remote_version = luci.sys.exec("echo -n $(curl -fsSL http://192.168.123.2/version.txt) | tr -d '\n'")
		updatelogs = luci.sys.exec("echo -n $(curl -fsSL http://192.168.123.2/release.txt) | tr -d '\n'")
...
function to_check()
	...
	if model == "x86_64" then
		...
		if fs.access("/sys/firmware/efi") then
			download_url = "http://192.168.123.2/openwrt-x86-64-generic-squashfs-combined-efi.img.gz"
		else
			download_url = "http://192.168.123.2/openwrt-x86-64-generic-squashfs-combined.img.gz"
		end
	else
